/* Упражнение 02.
А) Перед main() в комментариях ответьте на вопросы и напишите соответствия С – С++ для следующих операций :
...
int main() {
    ...
    // Создание треугольника Фибоначчи - Creación del triángulo de Fibonacci
    int** fibonacciTriangle = createFibonacciTriangle(rows);

    // Вывод треугольника Фибоначчи - Conclusión del triángulo de Fibonacci
    printFibonacciTriangle(fibonacciTriangle, rows);

    // Освобождение памяти - Liberación de memoria
